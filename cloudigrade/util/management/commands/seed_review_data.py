...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        account_helper.generate_instance_type_definitions()
        ...
        # Clounts
        user1clount1 = account_helper.generate_cloud_account(
            user=user1, created_at=two_years, name="user1clount1"
        )
        user1clount2 = account_helper.generate_cloud_account(
            user=user1, created_at=two_years, name="user1clount2"
        )

        user2clount1 = account_helper.generate_cloud_account(
            user=user2, created_at=two_years, name="user2clount1"
        )
        user2clount2 = account_helper.generate_cloud_account(
            user=user2, created_at=two_years, name="user2clount2"
        )
        user2clount3 = account_helper.generate_cloud_account(
            user=user2,
        ...
        user2clount1image1 = account_helper.generate_image(
            owner_aws_account_id=int(user2clount1.content_object.aws_account_id),
        )
        user2clount2image1 = account_helper.generate_image(
            owner_aws_account_id=int(user2clount1.content_object.aws_account_id),
            ec2_ami_id="ami-8f6ad3ef",
            openshift_detected=True,
        )
        user2clount3image1 = account_helper.generate_image(
            owner_aws_account_id=int(user2clount1.content_object.aws_account_id),
            is_cloud_access=True,
        )
        user2clount3image2 = account_helper.generate_image(
            owner_aws_account_id=int(user2clount1.content_object.aws_account_id),
            is_marketplace=True,
        )
        ...
        # Instances
        # User 1 Clount 1
        user1clount1image1instance1 = account_helper.generate_instance(
            cloud_account=user1clount1, image=user1clount1image1,
        )
        # User 1 Clount 2
        user1clount2image1instance1 = account_helper.generate_instance(
            cloud_account=user1clount2, image=user1clount2image1,
        )
        user1clount2image2instance1 = account_helper.generate_instance(
            cloud_account=user1clount2, image=user1clount2image2,
        )
        user1clount2image3instance1 = account_helper.generate_instance(
            cloud_account=user1clount2, image=user1clount2image3,
        )

        # User 2 Clount1
        user2clount1image1instance1 = account_helper.generate_instance(
            cloud_account=user2clount1, image=user2clount1image1,
        )
        user2clount1image1instance2 = account_helper.generate_instance(
            cloud_account=user2clount1, image=user2clount1image1,
        )
        user2clount1image1instance3 = account_helper.generate_instance(
            cloud_account=user2clount1, image=user2clount1image1,
        )
        # User 2 Clount 2
        user2clount2image1instance1 = account_helper.generate_instance(
            cloud_account=user2clount2, image=user2clount2image1,
        )
        user2clount2image1instance2 = account_helper.generate_instance(
            cloud_account=user2clount2, image=user2clount2image1,
        )
        # User 2 Clount 3
        user2clount3image1instance1 = account_helper.generate_instance(
            cloud_account=user2clount3, image=user2clount3image1,
        )
        user2clount3image2instance1 = account_helper.generate_instance(
